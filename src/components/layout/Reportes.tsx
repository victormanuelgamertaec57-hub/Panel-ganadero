  ...
  return (
    <div>
      ...
      <div ref={reportRef} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {/* Header del reporte */}
        <div className="card" style={{ padding: '22px 24px', background: 'linear-gradient(135deg, #1C1F2E 0%, #252840 100%)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
            <svg width="44" height="44" viewBox="0 0 100 100"><rect width="100" height="100" rx="18" fill="#4CAF82"/><path d="M25 40 C20 30 18 22 28 20 C32 19 35 24 35 28 C35 24 38 19 42 20 L50 20 C54 19 57 24 57 28 C57 24 60 19 64 20 C74 22 72 30 67 40 L65 45 C68 48 70 52 70 57 C70 70 61 78 50 78 C39 78 30 70 30 57 C30 52 32 48 35 45 Z" fill="white"/></svg>
            <div>
        ...
        <div className="card" style={{ padding: '20px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {kpiCard('Ventas individuales', formatCOP(totalVentas), '#4CAF82')}
              ...
              <table className="data-table">
                ...
                <tbody>
                  {filteredVentas.map(v => (
                    <tr key={v.id}>
                      ...
                      <td style={{ textAlign: 'right', fontFamily: 'IBM Plex Mono', fontSize: '0.82rem' }}>{v.peso_kg.toLocaleString('es-CO')}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: '#4CAF82', fontFamily: 'IBM Plex Mono' }}>{formatCOP(v.valor_total)}</td>
                ...
                <tfoot>
                  <tr style={{ backgroundColor: 'var(--color-surface-2)' }}>
                    ...
                    <td style={{ textAlign: 'right', fontWeight: 700, color: '#4CAF82', fontFamily: 'IBM Plex Mono', padding: '10px 14px' }}>{formatCOP(totalVentas)}</td>
                    <td colSpan={2} />
                  </tr>
                </tfoot>
              </table>
                ...
                <tbody>
                  {filteredLotes.map(l => (
                    <tr key={l.id}>
                      ...
                      <td style={{ textAlign: 'right', fontFamily: 'IBM Plex Mono' }}>{calcTotalCabezas(l)}</td>
                      <td style={{ textAlign: 'right', fontFamily: 'IBM Plex Mono', color: 'var(--color-text-secondary)' }}>{formatCOP(l.total_bruto)}</td>
                      <td style={{ textAlign: 'right', fontFamily: 'IBM Plex Mono', color: '#E05C5C' }}>{formatCOP(l.total_gastos)}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: '#4CAF82', fontFamily: 'IBM Plex Mono' }}>{formatCOP(l.total_neto)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
